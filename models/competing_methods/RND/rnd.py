...
from .unet import Unet_3D_noise_frecom_DWTIWT_WTConv3d_decoder
...
class MHSCA3D(nn.Module):
    def __init__(self, channels=16, num_heads=1):
        super(MHSCA3D, self).__init__()
        self.num_heads = num_heads
        self.temperature = nn.Parameter(torch.ones(1, num_heads, 1, 1))
        self.temperature2 = nn.Parameter(torch.ones(1, num_heads, 1, 1))

        self.qkv = nn.Conv3d(channels, channels * 3, kernel_size=(1, 1, 1), bias=False)
        self.qkv_conv = nn.Conv3d(channels * 3, channels * 3, kernel_size=(1, 3, 3), padding=(0, 1, 1), groups=channels * 3, bias=False)# 331
        self.qkv2 = nn.Conv3d(channels, channels * 3, kernel_size=(1, 1, 1), bias=False)
        self.qkv_conv2 = nn.Conv3d(channels * 3, channels * 3, kernel_size=(1, 3, 3), padding=(0, 1, 1), groups=channels * 3, bias=False)# 331
        self.project_out = nn.Conv3d(channels * 2, channels, kernel_size=(1, 1, 1), bias=False)
    ...
    def forward(self, x):
        ...
        q_c, k_c, v_c = self.qkv_conv(self.qkv(x)).chunk(3, dim=1)

        # channel
        q_c = q_c.reshape(b, self.num_heads, -1,  h * w * c)
        k_c = k_c.reshape(b, self.num_heads, -1,  h * w * c)
        v_c = v_c.reshape(b, self.num_heads, -1,  h * w * c)
        q_c, k_c = F.normalize(q_c, dim=-1), F.normalize(k_c, dim=-1)

        attn_c = torch.softmax(torch.matmul(q_c, k_c.transpose(-2, -1).contiguous()) * self.temperature2, dim=-1)

        out = self.project_out(torch.cat([torch.matmul(attn, v).reshape(b, t, -1, h, w), torch.matmul(attn_c, v_c).reshape(b,-1,c,h,w)], dim=1))
        return out
...
class RND_TDSAT(nn.Module):
    def __init__(self, in_channels, channels, num_half_layer, sample_idx):
        super(RND_TDSAT, self).__init__()
        self.net_first = Unet_3D_noise_frecom_DWTIWT_WTConv3d_decoder(1, 12)
        self.net_second = TDSAT(in_channels, channels, num_half_layer, sample_idx)
        self.net_second.use_2dconv = False
        self.net_second.bandwise = False

    def forward(self, x):
        out1 = self.net_first(x)
        out = self.net_second(out1)
        return out1, out
